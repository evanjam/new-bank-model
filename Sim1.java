...
{	
	...
	public static void execute(Event e)
	{
		...
		case 1:
			...
			TT = TT + tt; 
			count++;
			System.out.println("\n******\nType " + e.type + " event (exit from teller " + e.type + ")\narrTime: " + C1.getArr() + ", serTime = " + C1.getSer() + ", waiTime = " + C1.getWai() + ", at time: " + CURR);
			...
			{
				C1 = Qcust.dequeue();
				if(C1.getArr() + C1.getWai() < CURR)
				{
					System.out.println("Cust already left bank.");
				}
				else
				{
					next = new Event(CURR + C1.getSer(), 1);
					Elist.insert(next);
					B1 = 1;
				}
			}
		...
		case 2:
			...
			TT = TT + tt; 
			count++;
			System.out.println("\n******\nType " + e.type + " event (exit from teller " + e.type + ")\narrTime: " + C2.getArr() + ", serTime = " + C2.getSer() + ", waiTime = " + C2.getWai() + ", at time: " + CURR);
			System.out.println("cust throughtime tt: " + tt);
			System.out.println("total throughtime TT: " + TT);
			System.out.println(count + " customers have exited the bank");
			if(Qcust.getSize() > 0)
			{
				C2 = Qcust.dequeue();
				if(C2.getArr() + C2.getWai() < CURR)
				{
					System.out.println("***Cust already left bank.");
				}
				else
				{
					next = new Event(CURR + C2.getSer(), 2);
					Elist.insert(next);
					B2 = 1;
				}
			}
		...
		case 5: 
			...
			else
			{
				if(Qcust.getSize() > 0)
				{
					...
					{
						next = new Event(CURR + C3.getSer(), 3);
						Elist.insert(next);
						B3 = 1;
					}
				}
		...
		case 7:
			Qcust.enqueue(CUST);
			System.out.println("\n******\nType 7 Event:\nCust Arrived from file: " + CUST.getArr() + " " + CUST.getSer() + " " + CUST.getWai());
			System.out.println("Time: " + CURR);
			if(CustArr.getSize() > 0)
			{
				CUST = CustArr.dequeue();
				next = new Event(CUST.getArr(), 7);
				Elist.insert(next);
			}
			if(Qcust.getSize() == 1)
			{
	...
	public static void main(String[] args)
	{
		...
		//code to step through event execution (for testing)
/**		Event e = Elist.getData(1);
		CURR = e.getTime();
		execute(e);
		Elist.del(Elist.getData(1));
		Elist.printList();
**/	
